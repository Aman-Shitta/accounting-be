...
# DO NOT CHANGE
# Static code works  as per current setup
...
def process_document_ai(
    ...
    try:
        ...
        logger.info(
            f"Exception in process bytes for rectify process document ai: {exc_type}, File: {fname}, Line: {exc_tb.tb_lineno}")
...
- check images embedded in the statement
...
"""
Helpers to stitch split rows (date/desc in one row, amount in next)
"""
...
"""
More robust Gemini response text extraction
"""
...
def _truncate_to_2_decimals(value: float) -> float:
    """Pass through the float value without manipulation."""
    return float(value)
...
def process_bytes(file_bytes: str, debug_storage=None):
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        logger.info(
            f"Exception in process bytes for rectify document: {exc_type}, File: {fname}, Line: {exc_tb.tb_lineno}")
        logger.info(f"Error: {e}")

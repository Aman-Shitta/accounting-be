...
class ToolResourcesCodeInterpreter():
        ...
        try:
            ...
            for idx, assistant in enumerate(assistants.data, 1):
                created_at = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.localtime(assistant.created_at))
                ...
                print(
                    f"   Tools: {', '.join(assistant_info['tools']) or 'None'}")
                print(
                    f"   Vector Store IDs: {', '.join(vector_store_ids) or 'None'}")
                print("-" * 40)
    ...
    def delete_assistant(self, assistant_id):
        ...
        try:
            ...
            print(
                f"Vector Store IDs: {', '.join(details['vector_store_ids']) or 'None'}")
            print("=" * 60)
            ...
            try:
                DimAICAssistant.objects.filter(
                    assistant_id=assistant_id).delete()
    ...
    def interactive_assistant_manager(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                if vs_id:
                    confirm = input(
                        f"Are you sure you want to delete vector store {vs_id}? (yes/no): ").strip().lower()
                    if confirm == 'yes':
                        self.delete_vector_store(vs_id)
                    else:
                        print("Deletion cancelled.")
    ...
    def list_all_vector_stores(self):
        ...
        try:
            ...
            for idx, vs in enumerate(vector_stores.data, 1):
                created_at = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.localtime(vs.created_at))
                last_active = time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.localtime(vs.last_active_at)) if vs.last_active_at else 'Never'

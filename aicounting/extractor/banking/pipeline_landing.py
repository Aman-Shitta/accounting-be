...
class DocumentProcessor(BaseDocumentProcessor):
    ...
    def process_document(self, file_bytes: bytes, **kwargs):
        ...
        for i, page_bytes in enumerate(page_bytes_list):
            ...
            try:
                ...
                page_types = self.page_classifier.classify(
                ...
                page_types_names = [pt['type'] if isinstance(
                    pt, dict) else pt for pt in page_types]

                logger.info(
                    f"[DEBUG] Page {page_num} types names: {page_types_names}")
                check_key = None
        ...
        if self.control_totals and self._is_summary_incomplete():
            logger.info(
                "Summary is incomplete, attempting re-extraction from combined pages")
            relevant_pages = self._get_relevant_pages()
            if relevant_pages:
                enhanced_summary = self._extract_summary_from_combined_pages(
                    relevant_pages)
                if enhanced_summary:
                    self.control_totals = enhanced_summary
                    logger.error(
                        "Successfully re-extracted summary from combined pages")
    ...
    def _save_control_totals(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to convert control totals for saving: {self.control_totals} : {e}")
    ...
    def _save_doc_metadata(self):
        ...
        try:
            self.document.markdown_metadata = convert_decimals_to_float(
                self.pages_data)
            self.document.save()
        except Exception as e:
            logger.error(
                f"Failed to convert markdown metadata for saving: {self.pages_data} : {e}")
        ...
        with transaction.atomic():
            for page_idx, page_content in self.pages_data.items():
                ...
                for line_idx, line_item in enumerate(line_items_data):
                    ...
                    check_num = str(check_num).lstrip('0').strip(
                        '*') if check_num else check_num
                    ...
                    if check_num and str(check_num).strip() and (check_num in checks_linked):
                        if len(checks_linked[check_num].description) > len(line_item.get("description", "")):
                            ...
                            checks_linked[check_num].description = line_item.get(
                                "description", "")
                            continue

                    line_item_obj = self._save_line_item(
                        page_idx, line_idx + 1, line_item)
                ...
                for check_item in table_checks:
                    ...
                    if check_obj is None:
                        continue
                    stats["check_items"] += 1
                    self._link_check_to_line_item(
                        check_obj, create_if_missing=True)
                ...
                for check_item in image_checks:
                    ...
                    self._link_check_to_line_item(
                        check_obj, create_if_missing=False)
    ...
    def _save_line_item(self, page_number: int, line_number: int, line_data: Dict) -> MonthlyDocumentBankLineItem:
        ...
        is_check_transaction = line_data.get(
            "is_check_transaction", False) is not None
        ...
        rectification_reasoning = line_data.get(
    ...
    def _save_check_item(self, page_number: int, check_data: Dict) -> MonthlyDocumentBankCheckItem:
        ...
        check_number = str(check_number).lstrip('0').strip(
            '*') if check_number else check_number
        ...
        parsed_clearing_date = self.format_date(check_data.get(
            "clearing_date", "")) if check_data.get("clearing_date", "") else ""
        parsed_passing_date = self.format_date(check_data.get(
            "passing_date", "")) if check_data.get("passing_date", "") else ""
    ...
    def _link_check_to_line_item(self, check_item: MonthlyDocumentBankCheckItem, create_if_missing: bool = True):
        ...
        if matching_line_item:
            ...
            logger.info(
                f"Check #{check_item.check_number} not found in line items. Skipping creation as create_if_missing=False.")

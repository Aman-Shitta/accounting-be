...
class AmountGrounding(BaseModel):
    left: Annotated[Decimal,
                    Field(..., description="left location coordinate of the amount on the page.")]
    top: Annotated[Decimal,
                   Field(..., description="Top location coordinate of the amount on the page.")]
    right: Annotated[Decimal, Field(
        ..., description="Right location coordinate of the amount amount on the page.")]
    bottom:  Annotated[Decimal, Field(
        ..., description="Bottom location coordinate of the amount amount on the page.")]


class StatementTransaction(BaseModel):
    global_id: int = Field(
        ..., description="Numerical unique identifier indicating the order as listed on the statement.")
    date: str = Field(...,
                      description="Transaction or check Date in mm/dd/yyyy format.")
...
class BankStatementExtraction(BaseModel):
    transactions: List[StatementTransaction] = Field(
        ..., description="An array of all transactions and checks listed in the statement, in the order they appear.")

...
class DocumentConfig:
    ...
    def validate(self) -> None:
        ...
        if self.doc_type not in valid_types:
            raise ValueError(f"Invalid doc_type: {self.doc_type}. Must be one of {valid_types}")
        
        if self.doc_type in DocumentType.transactional_types():
            if not self.extract_line_items:
                raise ValueError(f"extract_line_items must be True for {self.doc_type}")
        
        if self.doc_type in DocumentType.attribute_types():
            if not self.extract_key_items:
                raise ValueError(f"extract_key_items must be True for {self.doc_type}")

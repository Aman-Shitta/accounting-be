...
class JSONHelper:
    ...
    def clean(raw: str) -> str:
        ...
        try:
            ...
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            logger.error(f"[{fname}:{exc_tb.tb_lineno}] Error cleaning JSON: {e}")
    ...
    def extract_first_json(raw: str) -> str:
        ...
        try:
            ...
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            logger.error(f"[{fname}:{exc_tb.tb_lineno}] Error extracting JSON: {e}")
    ...
    def repair(raw: str) -> str:
        ...
        try:
            ...
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            logger.error(f"[{fname}:{exc_tb.tb_lineno}] JSON repair failed: {e}")
            return raw
        ...
        if expected_keys:
            for key, default_value in expected_keys.items():
                if key not in parsed:
                    logger.warning(f"Missing expected key '{key}', using default")

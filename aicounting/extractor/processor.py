    ...
    def register(cls, *doc_types: str):
        ...
        def decorator(processor_class: Type):
            for doc_type in doc_types:
                ...
                logger.debug(
                    f"Registered {processor_class.__name__} for {doc_type}")
...
class MonthlyAccountingDocumentProcessor:
    ...
    def set_doc_processor(self, doc_type: str):
        ...
        self.doc_processor = processor_class(
            self.config, self.monthly_document)
    ...
    def start_process(self, file_bytes: str, mime_type: str = "application/pdf", md=False, special_rules="") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Document {self.monthly_document.id} processed successfully")
            ...
            self.monthly_document.save()
            logger.error(
                f"Failed to process document {self.monthly_document.id}: {str(e)}")
            raise
    ...
    def _clear_existing_data(self):
        ...
        MonthlyDocumentBankKeyItem.objects.filter(
            document=self.monthly_document).delete()
        MonthlyDocumentBankLineItem.objects.filter(
            document=self.monthly_document).delete()
        MonthlyDocumentBankCheckItem.objects.filter(
            document=self.monthly_document).delete()

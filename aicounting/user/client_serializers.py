...
class ClientCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    contact_name = serializers.CharField(
        required=True, max_length=100, help_text="Contact person's full name")
    contact_email = serializers.EmailField(
        required=True, max_length=100, help_text="Contact person's email address")
    ...
    chart_of_account = serializers.FileField(
        required=True, help_text="Chart Of Accounts file (CSV/Excel)")
    gl_history = serializers.FileField(
        required=False, help_text="General Ledger History file (CSV/Excel)")
    vendor_list = serializers.FileField(
        required=False, help_text="Vendor List file (CSV/Excel)")
    ...
    def validate_contact_phone(self, value):
        ...
        if len(value) > 15:
            raise serializers.ValidationError(
                "Phone number too long - maximum 15 characters including hyphens")
        ...
        if '-' in value:
            ...
            if len(parts) > 3:
                raise serializers.ValidationError(
                    "Invalid hyphen placement. Format: XXX-XXX-XXXX or XXXXXXXXXX")

            # Check each part's length if hyphens are present
            if not all(len(part) in [3, 4] for part in parts[:-1]) or len(parts[-1]) != 4:
                raise serializers.ValidationError(
                    "Invalid number grouping. Format: XXX-XXX-XXXX or XXXXXXXXXX")
    ...
    def validate_chart_of_account(self, value):
        ...
        if self.instance and self.instance.documents.filter(document_type='chart_of_account').exists():
            raise serializers.ValidationError(
                "A Chart Of Accounts document already exists for this client.")

        # Check file extension
        ext = os.path.splitext(value.name)[1].lower()
        if ext not in ['.csv', '.xls', '.xlsx']:
            raise serializers.ValidationError(
                "Chart Of Accounts file must be CSV or Excel format (.csv, .xls, .xlsx)")
        ...
        try:
            ...
            try:
                ...
                if len(df) == 0:
                    raise serializers.ValidationError(
                        "The file is empty. Please upload a file with data.")
    ...
    def validate_gl_history(self, value):
        ...
        if self.instance and self.instance.documents.filter(document_type='gl_history').exists():
            raise serializers.ValidationError(
                "A Ledger History document already exists for this client.")

        ext = os.path.splitext(value.name)[1].lower()
        if ext not in ['.csv', '.xls', '.xlsx']:
            raise serializers.ValidationError(
                "GL History file must be CSV or Excel format (.csv, .xls, .xlsx)")
        ...
        try:
            ...
            logger.error(
                f"Error validating Ledger history file structure: {e}")
            raise serializers.ValidationError(f"Error reading file: {str(e)}")
    ...
    def validate_vendor_list(self, value):
        ...
        if self.instance and self.instance.documents.filter(document_type='vendor_list').exists():
            raise serializers.ValidationError(
                "A Vendor List document already exists for this client.")

        ext = os.path.splitext(value.name)[1].lower()
        if ext not in ['.csv', '.xls', '.xlsx']:
            raise serializers.ValidationError(
                "Vendor List file must be CSV or Excel format (.csv, .xls, .xlsx)")
        try:
            # Save uploaded file to a temporary location for validation
            with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as temp_file:
                for chunk in value.chunks():
    ...
    def __validate_document__(self, data, client, is_update=False):
        ...
        if not any(data.values()):
            if not is_update:
                raise serializers.ValidationError(
                    "At least one document file must be provided.")
                        ...
                        try:
                            ...
                            with default_storage.open(document.file.name, 'rb') as azure_file:
                                ...
                                with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as temp_file:
                                    ...
                                    temp_file_path = temp_file.name
                                try:
                                    result = client_doc_processor.process_document(
                                        temp_file_path, doc_type)
                                except Exception as e:
                                    # Clean up temp file
                                    if os.path.exists(temp_file_path):
                                        os.remove(temp_file_path)
                                    ...
                                    if doc.file:
                                        try:
                                            default_storage.delete(
                                                doc.file.name)
                                            logger.error(
                                                f"Cleaned up file from Azure: {doc.file.name}")
                                        except Exception as cleanup_error:
                                            logger.error(
                                                f"Error cleaning up file {doc.file.name}: {cleanup_error}")
                            ...
                            if result['errors']:
                                logger.error(
                                    f"Processing completed with {len(result['errors'])} errors: {result['errors']}")
                        except serializers.ValidationError:
                            raise
                        except Exception as e:
                            logger.error(
    ...
    def update(self, instance, validated_data):
        ...
        with transaction.atomic():
            try:
                ...
                existing_contact = DimAICContact.objects.filter(
                    client_id=instance).first()

                if existing_contact:
                    # Update existing contact
                    existing_contact.contact_name = contact_data.get(
                        'contact_name', existing_contact.contact_name)
                    existing_contact.contact_email = contact_data.get(
                        'contact_email', existing_contact.contact_email)
                    existing_contact.contact_phone = contact_data.get(
                        'contact_phone', existing_contact.contact_phone)
                    existing_contact.save()
                    logger.error(
                        f"Updated existing contact for client {instance.client_id}")
                else:
                    # Create new contact
                    ...
                    logger.error(
                        f"Created new contact for client {instance.client_id}")
                ...
                for doc_type, file_obj in documents.items():
                    if file_obj:
                        ...
                        if existing_doc:
                            raise serializers.ValidationError({
                                doc_type: [
                                    f"A document of type '{doc_type}' already exists for this client."]
                            })
                        ...
                        try:
                            ...
                            if not result['success']:
                                ...
                                raise Exception(
                                    f"Processing failed for {doc_type}: {result.get('error', 'Unknown error')}")
...
class ClientAccountantAssignmentSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        current_accountants = set(
            instance.assigned_accountants.values_list('id', flat=True))
                ...
                for doc_type, file_obj in validated_data.items():
                    if file_obj:
                        ...
                        try:
                            ...
                            raise Exception(
                                f"Document processing failed for {doc_type}: {str(e)}")

        except Exception as e:
            logger.error(f"Error creating/processing documents: {str(e)}")
            raise serializers.ValidationError(
                f"Document creation and processing failed: {str(e)}")

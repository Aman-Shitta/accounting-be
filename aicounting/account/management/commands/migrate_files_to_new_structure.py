...
from account.models.dim_aic_snapshot_models import FactAICInputFileSnapshot, FactAICJETemplateHeaderSnapshot
...
class Command(BaseCommand):
    ...
    def migrate_snapshots(self):
        ...
        for item in qs:
            self._migrate_file(
                item,
                'file',
                lambda b, f, d: b.accounting_snapshots_path(f, add_timestamp=False, custom_date=d)
            )
    ...
    def migrate_je_templates(self):
        ...
        for item in qs:
            self._migrate_file(
                item,
                'je_export_file',
                lambda b, f, d: b.je_template_path(f, add_timestamp=False, custom_date=d)
            )
    ...
    def migrate_processing_artifacts(self):
        ...
        for doc in qs:
            ...
            date_obj = doc.created_at if hasattr(doc, 'created_at') else datetime.now()
            date_str = date_obj.strftime("%Y-%m-%d")
            ...
            try:
                ...
                if hasattr(default_storage, 'blob_service_client'):
                    container_client = default_storage.blob_service_client.get_container_client(default_storage.container_name)
                    ...
                    for blob in blobs:
                        ...
                        self.stdout.write(f"Artifact: {old_blob_name} -> {new_path}")
                        ...
                        if not self.dry_run:
                            try:
                                ...
                                self.stdout.write(self.style.ERROR(f"Failed artifact {old_blob_name}: {e}"))
                                
            except Exception as e:
                self.stdout.write(self.style.WARNING(f"Could not list artifacts for {doc_id}: {e}"))

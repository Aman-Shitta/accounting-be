...
class JETemplateDataSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'je_name', 'je_refrence', 'description',
            'input_files', 'je_freq', 'is_object', 'input_files'
        ]

    def to_representation(self, instance):
        """
        Convert JE template instance to JSON representation.
        Handles different document types: bank statements, credit cards, 
        and other documents.
        """
        ...
        ret['attributes'] = JETemplateAttributeDataSerializer(
            attributes_data,
            many=True
        ).data
    ...
    def _calculate_is_verified(self, instance):
        ...
        if not instance.je_export_file:
            return False
        ...
        if instance.status == 'verified' and all_verified:
            if instance.je_export_file:
                return True
    ...
    def _handle_single_attribute_template(self, instance):
        """
        Handle JE templates with a single attribute (typically banking).
    ...
    def _process_bank_documents(self, input_file_snapshot):
        ...
        for item in items:
            attributes_data.append({
                ...
                "is_editable": "debit" if item.transaction_type == "debit" else "credit"
            })
    ...
    def _handle_multi_attribute_template(self, instance):
        ...
        if not input_file_snapshot:
            ...
            attributes_data = self._get_no_file_attributes(instance)
        else:
            ...
            if instance.is_object:
                ...
                attributes_data = self._process_non_object_template(
                    instance, all_documents
                )

        return attributes_data

    def _get_no_file_attributes(self, instance):
        """
    ...
    def _get_attribute_value(self, template_attr, all_documents):
        ...
        logger.error(
            "template_attr.input_file_attribute :: ",
            template_attr.input_file_attribute,
            template_attr.attribute_name
        )
        ...
        if template_attr.input_file_attribute:
            ...
            attribute_value = self._get_or_create_manual_attribute(
                template_attr)
